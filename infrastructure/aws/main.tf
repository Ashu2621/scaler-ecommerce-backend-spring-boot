...
resource "aws_ecs_task_definition" "app" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "DATABASE_URL", value = "jdbc:mysql://${aws_db_instance.mysql.address}:3306/ecommerce?useSSL=true&serverTimezone=UTC" },
      { name = "DATABASE_USERNAME", value = aws_db_instance.mysql.username },
      { name = "REDIS_HOST", value = aws_elasticache_replication_group.redis.primary_endpoint_address },
      { name = "SPRING_DATA_REDIS_SSL_ENABLED", value = "true" },
      { name = "MESSAGING_PROVIDER", value = "local" }
    ]
    secrets = [
      {
